...
$env:RUSTFLAGS="-C target-cpu=cortex-a715 -C target-feature=+crc -C link-arg=-Wl,--gc-sections"
...
Then per-KMI (copy the exact `*.ko` as generic name):

```sh
Get-ChildItem userspace/ksud/bin/aarch64/*.ko | Remove-Item -Force
Copy-Item G:\...\kernelsu\android13-5.15.189_kernelsu-f946b-F946BXXS7GZE5.ko userspace/ksud/bin/aarch64/android13-5.15.189_kernelsu.ko
...
# repeat for e2s (android14-6.1), s25u (android15-6.6), etc. — 12 binaries
# sizes now ~5.39M (O2) vs 4.88M (Oz)
```

`android12-5.10`/`android14-6.1` generic and `A366/A566` `android15-6.6` each need their own `ko → androidXX_kernelsu.ko` copy before `cargo build`.

### 2.3 Feed & dist

```sh
python update_feed.py  # reads artifacts/*/size + kernelsu/ksud-*/size → support/targets-v3.json
# then sync dist/ as release.yml does:
