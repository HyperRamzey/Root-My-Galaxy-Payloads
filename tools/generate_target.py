...
# ---------------------------------------------------------------------------
# adb-exploit pipeline integration (Root-My-Galaxy flow)
#
# The app + helper pipeline is adb-based: the RootOnBootService (or a manual
# run) stages cve-2026-43499-app.so / cve-2026-43499-root / ksud-*-kdp into
# /data/local/tmp over wireless adb, then launches
#   <env vars> cve-2026-43499-root --run-payload f946b.so \
#       cve-2026-43499-root <log>
# and waits for "exploit completed" on stdout / in the log. The emitters
# below keep generated profiles wired into that exact flow.
# ---------------------------------------------------------------------------
...
def emit_quickstart(profile, build, fingerprint, model):
    """Print the manual one-attempt-per-boot block for this profile."""
    print(f"""
### {model or build} — `{profile}` quick start (one attempt per boot)

```sh
adb push artifacts/{profile}/cve-2026-43499-app.so /data/local/tmp/{profile.split('-')[0]}.so
adb push artifacts/{profile}/cve-2026-43499-root  /data/local/tmp/cve-2026-43499-root
...
adb shell "SLIDE_SOURCE=tracefs EXPLOIT_ATTEMPTS=1 P0_ATTEMPT_TIMEOUT_SEC=115 \\
  ...
  /data/local/tmp/{profile.split('-')[0]}.so \\
  /data/local/tmp/cve-2026-43499-root /data/local/tmp/{profile.split('-')[0]}.log"
```

KernelSU late-load then runs through the root daemon's activation watcher
(or manually: `adb shell /data/local/tmp/cve-2026-43499-root --late-load`).
Build id: `{build}`; fingerprint: `{fingerprint}`.
""")
...
def update_feed(feed_path, profile, model, build, kernel_release,
    ...
    artifacts/<profile>/ when built (exploit is mandatory for a feed entry,
    helper/ksud are optional because the app falls back to its bundled
    helper). Existing entries keep their payloadId so app-side caches stay
    valid across re-runs.
    """
    import json
    raw = feed_path.read_text(encoding='utf-8')
    feed = json.loads(raw)
    if feed.get('schemaVersion') != 3:
        raise SystemExit(f"{feed_path}: unexpected schemaVersion "
    ...
    def artifact(name, rel):
        p = ARTIFACT_DIR / profile / name
        if not rel or not p.exists():
            return None
        return {'url': base + p.as_posix(), 'size': p.stat().st_size}

    exploit = artifact('cve-2026-43499-app.so', True)
    if exploit is None:
        print(f"feed: skipped ({ARTIFACT_DIR / profile / 'cve-2026-43499-app.so'} "
              f"not built yet); generate it with `make TARGET={profile}`")
        return
    helper = artifact('cve-2026-43499-root', True)

    ksud = None
    for cand in sorted((Path('kernelsu')).glob(f'ksud-{profile}-kdp')):
        ksud = {'url': base + cand.as_posix(), 'size': cand.stat().st_size}
        break
    ...
    entry = {
        'payloadId': profile,
        'displayName':
            f"{model or build} | Kernel {'/'.join([kernel_release])}",
        'models': [model] if model else [],
        'kernelVersions': [kernel_release],
        'exploit': exploit,
        'kernelsu': ksud,
        'slideSource': slide_source,
        'notes': f"Generated by tools/generate_target.py; build {build}.",
    }
    if helper:
    ...
    feed_path.write_text(json.dumps(feed, indent=2, ensure_ascii=False) +
                         "\n", encoding='utf-8')
    print(f"feed: wrote {entry_id} -> {feed_path}")


def adb_launch(profile, serial, attempt_timeout):
    """Validate the generated profile end-to-end over adb.

    Pushes the freshly built helper + payload from build/<profile>/ to
    /data/local/tmp exactly like RootOnBootService does, runs one exploit
    attempt and streams output until the success marker appears. Read-only
    diagnostics afterwards: no reboot, no late-load (those belong to the
    app's auto-root pipeline).
    """
    import subprocess
    adb = ['adb'] + (['-s', serial] if serial else [])
    helper = BUILD_DIR / profile / 'cve-2026-43499-root'
    payload = BUILD_DIR / profile / 'cve-2026-43499-app.so'
    for p in (helper, payload):
        if not p.exists():
            raise SystemExit(f"--adb-launch: missing {p}; run "
                             f"`make TARGET={profile}` first")
    remote_helper = '/data/local/tmp/cve-2026-43499-root'
    remote_payload = f'/data/local/tmp/{profile.split("-")[0]}.so'
    remote_log = f'/data/local/tmp/{profile.split("-")[0]}.log'

    def sh(cmd, timeout=60):
        r = subprocess.run(adb + ['shell', cmd], text=True,
                           capture_output=True, timeout=timeout)
        return r.stdout + r.stderr

    print(f"[adb] staging {helper.name}, {payload.name}")
    subprocess.run(adb + ['push', str(helper), remote_helper], check=True)
    subprocess.run(adb + ['push', str(payload), remote_payload], check=True)
    sh(f"chmod 755 {remote_helper}; rm -f {remote_log}")

    env = ("SLIDE_SOURCE=tracefs EXPLOIT_ATTEMPTS=1 "
           "P0_ATTEMPT_TIMEOUT_SEC=115 EXPLOIT_ATTEMPT_TIMEOUT_SEC="
           f"{attempt_timeout} ")
    cmd = (f"{env}{remote_helper} --run-payload {remote_payload} "
           f"{remote_helper} {remote_log}")
    print(f"[adb] running: {cmd}")
    proc = subprocess.Popen(adb + ['shell', cmd], stdout=subprocess.PIPE,
    ...
    try:
        for line in proc.stdout:
            ...
            if time.time() > deadline:
                proc.kill()
                print("[adb] timeout before success marker")
                return 1
    finally:
        proc.wait(timeout=30)

    tail = sh(f"tail -5 {remote_log} 2>/dev/null")
    ok = 'exploit completed' in (tail + '')
    print(f"[adb] result: {'SUCCESS' if ok else 'no success marker'}")
...
def main():
    ...
    ap.add_argument('--feed', type=Path,
                    help="insert/update this profile's entry in the "
                         "support/targets-v3.json feed consumed by the app")
    ap.add_argument('--quickstart', action='store_true',
                    help="print the adb one-attempt-per-boot quick-start "
                         "block for the generated profile")
    ap.add_argument('--adb-launch', action='store_true',
                    help="validate end-to-end over adb: push helper+payload "
                         "from build/<profile>/ to a connected device, run "
                         "the exploit once and stream its log (requires "
                         "adb + built artifacts)")
    ap.add_argument('--adb-serial', default=None,
                    help="adb -s serial when several devices are attached")
    ap.add_argument('--attempt-timeout', type=int, default=600,
                    help="--adb-launch streaming timeout seconds "
                         "(default 600)")
    args = ap.parse_args()
    ...
    if args.quickstart:
        emit_quickstart(profile, build, fingerprint, args.model)
    if args.feed:
        update_feed(args.feed, profile, args.model, build, release,
                    slide_source='tracefs')
    if args.adb_launch:
        raise SystemExit(adb_launch(profile, args.adb_serial,
